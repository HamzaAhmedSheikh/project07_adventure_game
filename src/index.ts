...
async function welcome() {
    ...
    console.log(` ${chalk.hex("#ffffff").bold("In this cli base game, you have randomly generated three types of enemies Skeleton Warrior and Assassin")}`);
    console.log(` ${chalk.hex("#ffffff").bold("as a player you have three options attack an enemy, drink a potion, or run away from the enemy.")}`);
    console.log(` ${chalk.hex("#ffffff").bold("you just select one option given below.")}`);    
    console.log('');
...
let numHealthPotion  : number = 3; // numHealthPotion  
const healthPotionHealAmount: number = 30; // healthPotionHealAmount 
const healthPotionDorpChance: number = 50; // healthPotionDorpChance
...
async function endTheGame() {
    ...
    const endTheGame = chalkAnimation.karaoke(
        "--------------------------- The Game is Over ---------------------------------"
      );
